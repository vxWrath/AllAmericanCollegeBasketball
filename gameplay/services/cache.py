...
import os
import sys
from typing import Any, Dict, Iterable, Optional, Set, Tuple, Type, Union, get_origin
...
class Cache:
    ...
    async def set(self, *path: Any, model: Union[msgspec.Struct, Dict[str, Any]], **kwargs: Any) -> None:
    ...
    async def get[T: Union[msgspec.Struct, Dict[str, Any]]](self, *path: Any, model_cls: Type[T], **kwargs: Any) -> Optional[T]:
    ...
    async def hash_set(self, *path: Any, model: Union[msgspec.Struct, Dict[str, Any]], keys: Iterable[str], **kwargs: Any) -> None:
        ...
        async with self.redis.pipeline() as pipe:
            await pipe.hset(name, mapping={k: orjson.dumps(v).decode() for k, v in data.items() if k in keys}) # type: ignore
    ...
    async def hash_get[T: Union[msgspec.Struct, Dict[str, Any]]](self, *path: Any, model_cls: Type[T], keys: Iterable[str]) -> Tuple[Optional[T], Set[str]]:
        ...
        for key, value in zip(ordered_keys, data):
            if value is not None:
                mapping[key] = orjson.loads(value)
            else:
                unretrieved_keys.add(key)

        logger.debug(f"Hash cache retrieved for key: {name!r}, keys: {list(mapping.keys())}, unretrieved: {list(unretrieved_keys)}")
